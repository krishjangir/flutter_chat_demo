...
class PhoneAuthDataProvider with ChangeNotifier {
  ...
  Future<FirebaseUser> getCurrentUser() async {
    FirebaseUser user = await FireBase.auth.currentUser();
    return user;
  }

  Future getUser() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    FirebaseUser firebaseUser = (await FireBase.auth.signInWithCredential(_authCredential)).user;
    if (firebaseUser != null) {
      ...
      if (documents.length == 0) {
        ...
        // Write data to local
        await prefs.setString(PrefKey.USER_ID, firebaseUser.uid);
        await prefs.setString(PrefKey.USER_NAME, firebaseUser.displayName);
        await prefs.setString(PrefKey.USER_PROFILE_PIC, firebaseUser.photoUrl);
      } else {
        // Write data to local
        await prefs.setString(PrefKey.USER_ID, documents[0]['id']);
        await prefs.setString(PrefKey.USER_NAME, documents[0]['nickname']);
        await prefs.setString(PrefKey.USER_PROFILE_PIC, documents[0]['photoUrl']);
        await prefs.setString(PrefKey.USER_ABOUT_ME, documents[0]['aboutMe']);
      }
    }
  }
